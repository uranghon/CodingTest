 ...
 */
public class Example_2156_포도주시식 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 3; i < n + 3; i++) {
            ...
            dp[i][0] = Math.max(dp[i - 3][0], Math.max(dp[i - 3][1], dp[i - 3][2])) + num;
            dp[i][1] = Math.max(dp[i - 2][0], Math.max(dp[i - 2][1], dp[i - 2][2])) + num;
            dp[i][2] = Math.max(dp[i - 1][0], dp[i - 1][1]) + num;
        }
        System.out.print(Math.max(dp[n + 1][2], Math.max(dp[n + 2][1], dp[n + 2][2])));
    }
}
