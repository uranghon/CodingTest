 ...
 *
 * -풀이-
 * 1. 최소단계를 복붙하기. -, - -, - -   - -, ... 보면 처음 문자열 에 이어 해당 문자열의 길이만큼 공백 이 있고, 그 뒤에 문자열을 복붙한다.
 *    이 상태를 또 한 문자열로 묶고, 해달 문자열 길이만큼 공백, 후 복붙. 이게 반복되는 형태이다.
 * 2. 설명에 나와있는거처럼 3^N 만큼 - 가 반복되는 문자열로 시작해서 가운데를 없애간다. 쪼개어져 들어갔을 때, 쪼갤 문자열 길이가 1이면 멈춘다.
 */
